...
def build_super_images(real_imgs, captions, ixtoword,
    ...
    bUpdate = 1
    for i in range(num): # !
        attn = attn_maps[i].cpu().view(1, -1, att_sze, att_sze) # ! tensor(1,cap_len,64/126,64/128),
        # --> 1 x 1 x 17 x 17
        attn_max = attn.max(dim=1, keepdim=True) # ! tensor(1,1,64/128,64/128),
        attn = torch.cat([attn_max[0], attn], 1) # ! tensor(1,cap_len+1,64/128,64/128),
        #
        attn = attn.view(-1, 1, att_sze, att_sze) # ! tensor(cap_len+1,1,64/128,64/128)
        attn = attn.repeat(1, 3, 1, 1).data.numpy() # ! tensor(cap_len+1,3,64/128,64/128), 
        # n x c x h x w --> n x h x w x c
        attn = np.transpose(attn, (0, 2, 3, 1)) # ! tensor(cap_len+1,64/128,64/128,3), 
        num_attn = attn.shape[0] # ! cap_len+1,
        #
        img = real_imgs[i] # 
        if lr_imgs is None:
            lrI = img
...
def build_super_images2(real_imgs, captions, cap_lens, ixtoword,
    ...
    for i in range(num):
        ...
        for j in range(num_attn):
            ...
            if maxV - minV == 0:
                RecursionError
            else: one_map = (one_map - minV) / (maxV - minV)
            row_beforeNorm.append(one_map)
